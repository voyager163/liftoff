...
Generated and reviewed adopted projects have two Liftoff root files with
different ownership. The current `0.13.0` writer is an unpublished development
candidate; its version is not a claim of release or platform qualification.

## `liftoff.config.json`: developer-owned desired state

Liftoff writes configuration once during initialization or explicitly reviewed
adoption and does not machine-rewrite it afterward.

Adopted components use the separate schema-1 `kind:
"liftoff-adopted-project"` desired-state form. It records the exact component
profile identities and roots, selected workflow/agents and governance preference,
not fabricated API, cloud or generation settings. Managed update preserves those
component identities; changing them is a separately reviewed project transition.
See [reviewed adoption](project-adoption.md).
...
## Generated API runtime configuration
...
Current writers use manifest artifact version 8; readers support historical manifest artifact version 7 and earlier formats. Its common project identity includes the
...
- `components`: the selected adopted component profiles and roots are in
  `standards`. This workload object contains only `kind`; a Vue-only project has
  no fabricated API stack, pattern, cloud, region, frontend toggle or environment.

The manifest also records:

- Last manifest-writing Liftoff version.
- Official framework adapter and actual state: `initialized`, retained
  historical `legacy` uncertainty, or adopted `uninitialized`. An uninitialized
  integration cannot claim configured agents or a tested installed contract.
- `standards`: schema-1 packaged profile/resource catalog digests and exact
  selected component identities. Target selection is not proof that existing
  code was generated by the target or satisfies every standard.
- `managedArtifacts`: exact Liftoff core logical names, paths, and
  reconciliation `contentHash` values.
- `projectArtifacts`: actual generation facts, adopted source observations, or
  explicitly reviewed project additions. Adopted entries link their original
  path/hash/mode to an independent adoption record without `generatedBy` or
  `generationHash`. Proposal-supplied additions identify that producer rather
  than claiming template generation. None of these hashes authorize update.
- `provenance`: original catalog generation or a byte-exact historical manifest,
  or independent adopted provenance. Registered subsequent manifest-writing
  repairs link their preserved source and unchanged schema-2 repair receipts.
- OS-neutral path-part arrays.
- Repository governance profile, current policy version 8, activation-contract
  version 4, state/evidence-header/approval schema versions 4,
  graph schema 3, credential-policy schema 2, unchanged supersession schema 1, the
  exact phase-graph hash, and local `handoff-generated`, `handoff-partial`, or
  disabled state.

The exact pre-amendment policy-7/schema-1 candidate remains readable with its
original manifest-8 provenance. Moving to policy 8 requires a reviewed
history-preserving transition and fresh credential approval; no original policy,
receipt or approval is retagged to authorize broader provider access.
...
Treat the manifest and its historical links as CLI-owned. Restore the exact
recorded metadata from version control or use its supported attributable recovery
when validation reports damage. Do not initialize over an existing application
or edit version, ownership, provenance or proof fields to clear an error.

Each manifest load shares one history-read budget: at most 8 MiB across 512
referenced files, with at most 4 MiB per history file and the existing
path/directory limits. Before returning trusted metadata, the reader rechecks
all consumed history files and parent directories for changes. An over-limit
or changed history fails explicitly; it is never truncated, partially trusted,
normalized, or repaired by a read-only inspection.

## Compatibility

Readers support artifact versions v2, v3, v4, v5, v6, and v7 for API/GenAI projects and accept strict current v8 generated and adopted forms:
...
- V6 separates managed-core update authority from project generation
  provenance.
- V7 retains its exact historical policy-6 activation identity, including the
  released activation-v1/v2/v3 tuples. Governance-disabled v7 manifests use the
  disabled variant and do not fabricate activation identity.
- V8 adds explicit selected profiles/components and generated/adopted provenance.
  Original historical profile facts remain explicitly unknown when they cannot
  be recovered; the current target is never substituted as the old producer.
...
historical v2-v7 manifests so `liftoff update --check` can report managed-core
drift and an exact-plan-approved update can migrate metadata to v8. Malformed or future manifest,
...
explicitly approved update writes v8 only through the guarded transaction,
preserving original bytes at the exact `.liftoff/manifest-history/<sha256>/manifest.json`
identity. V2-v6 backend, frontend, database, dependency, container,
...
V7's already-separated ownership and every recorded generation hash remain
unchanged, even for customized, relocated or missing application files.
...
| Governance output | 3 |
| Public protocol | 1 |
| Adoption plan, report, record and transaction | Independent schema 1 |
| Repair contract / report | 1 / 2 |
...
envelope version 3, compatibility metadata schema 4, and its original
29-phase canonical graph. Current independent infrastructure provenance
explicitly recognizes generation versions 0.11.0, 0.11.1, 0.11.2, 0.11.3, 0.12.0, 0.12.1, 0.12.2, 0.12.3, and the current 0.13.0 producer,
...
`governance/migration-state.json`; current execution needs fresh contract-4 proof.
Existing historical journals recover only their original recorded effects,
without upgrading their serializers or identities.
A readable historical record never authorizes current provider scope; old approvals
and checked tasks do not become fresh evidence. Revalidation failure after commit retains the actual successor
as blocked/resumable, not automatically rolled back or marked activated. Unknown or mixed tuples remain blocked.

Activation metadata migration and its local revalidation capture retained public
inputs with guarded file descriptors and a rechecked directory snapshot.
Declared retained state/key references and conventional protected payloads are
excluded before traversal or content reads. Links, special files, changed bytes,
changed modes or replaced parents block the operation instead of producing a
partial input binding.

These captures share the whole-project observation ceilings: 5,000 files,
2 MiB per file, 50 MiB total, directory depth 15, 10,000 traversed entries and a
15-second observation budget. Remaining byte limits apply before opening the
next file. The separate released application-preparation limits remain
512 files, 1 MiB per file and 8 MiB total. Retained-input fingerprints protect
the exact reviewed operation; they do not compare application files with
templates or grant application, script, provider or state-access authority.
...
- Writers use `artifactVersion` 8; isolated historical readers retain v2-v7.
