...
describe('infrastructure layout contract', () => {
  it.each(['0.11.0', '0.11.1', '0.11.2'])('recognizes complete independent roots generated by %s', (version) => {
    const identities = [
      ...sharedApplicationModuleIdentities,
      ...environmentRootInfrastructureIdentities('prod')
    ];
    const project = manifest(['prod'], identities.map(identity => ({
      ...recorded(identity), generatedBy: version
    })));
    project.liftoffVersion = version;
  ...
  it('accepts reviewed patch-generation mixtures without admitting unknown releases', () => {
    const project = manifest(['dev', 'staging', 'prod'], [
      ...sharedApplicationModuleIdentities.map(recorded),
      ...environmentRootInfrastructureIdentities('dev').map(recorded),
      ...environmentRootInfrastructureIdentities('staging').map(identity => ({
        ...recorded(identity), generatedBy: '0.11.1'
      })),
      ...environmentRootInfrastructureIdentities('prod').map(identity => ({
        ...recorded(identity), generatedBy: '0.11.2'
      }))
    ]);
    project.liftoffVersion = '0.11.2';
    expect(assessInfrastructureLayout(project).kind).toBe('independent');
    for (const unreviewed of ['0.10.4', '0.11.3', '0.12.0']) {
