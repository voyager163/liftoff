...
export type ExplicitArtifactLifecycleIdentity =
  (typeof explicitArtifactLifecycleIdentities)[number];

const explicitArtifactLifecycleIdentityMap: ReadonlyMap<
  string,
  ExplicitArtifactLifecycleIdentity
> = new Map(
  explicitArtifactLifecycleIdentities.map((identity) => [
    identity.logicalName,
    identity
  ])
);

export function explicitArtifactLifecycleIdentityFor(
  logicalName: string
): ExplicitArtifactLifecycleIdentity | undefined {
...
for (const environment of ['dev', 'staging', 'prod'] as const) {
  ...
  environmentProvisioningGroups.set(`environment-${environment}-backend`, group);
  environmentProvisioningGroups.set(`environment-${environment}-functions`, group);
  environmentProvisioningGroups.set(`opentofu-${environment}-tfvars`, group);
}
