...
describe('doctor command', () => {
  ...
  it('reports credential-expiring and identity-incompatible with exact remedies', async () => {
    ...
    expect(projectCheck(incompatible, 'governance-identity-incompatible')).toMatchObject({
      ...
      remedy: expect.stringContaining('Preserve user-owned state and evidence bytes')
    });
  }, 30_000);
  ...
  it('rejects a retired manifest before selecting checks or invoking probes', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'liftoff-retired-doctor-'));
    cleanups.push(root);
  ...
  it('does not report project template edits as managed-core drift', async () => {
    ...
    await rm(path.join(root, 'infrastructure', 'opentofu', 'azure', 'modules', 'application', 'main.tf'));
