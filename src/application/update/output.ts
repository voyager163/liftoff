...
import { createUpdateContinuation, formatUpdateCommand, type UpdateGuidanceContext } from './command-guidance.js';
...
export function buildUpdateReport(
  ...
  const continuations = inspection ? [
    createUpdateContinuation(input.projectRoot, 'check'),
    ...(input.committed ? [] : (input.plans ?? []).filter((plan) => plan.eligible && plan.writeCount > 0)
      .map((plan) => createUpdateContinuation(input.projectRoot, plan.mode)))
  ] : [];
  return {
...
function revalidationReviewDetails(revalidation: UpdateRevalidationSummary): string[] {
  ...
  return [
    ...
    ...(revalidation.issues.length ? ['Approval may commit the displayed successor identity while these known revalidation gaps remain blocked; preserved historical records are not current proof.'] : []),
    ...(preview ? [
...
export function renderUpdatePreview(
  ...
  presentation.definitions('Project versions', [
    ...
    { label: 'Manifest writer', value: inspection.manifest.liftoffVersion }
  ]);
