...
export function quoteCommandArgument(value: string, shell: CommandShell): string {
  return shell === 'posix' && /^[a-zA-Z0-9_./:@=-]+$/.test(value)
    ? value : literal(value, shell);
}

export function formatShellCommand(command: ExternalCommand, shell: CommandShell): string {
  const tokens = [command.executable, ...command.args].map((value) => quoteCommandArgument(value, shell));
  return `${shell === 'powershell' ? '& ' : ''}${tokens.join(' ')}`;
}

export function formatShellDirectoryCommand(command: ExternalCommand, cwd: string, shell: CommandShell): string {
  return formatShellDirectoryCommands([command], cwd, shell);
}

export function formatShellCommands(commands: readonly [ExternalCommand, ...ExternalCommand[]], shell: CommandShell): string {
...
export function formatShellDirectoryCommands(
  commands: readonly [ExternalCommand, ...ExternalCommand[]], cwd: string, shell: CommandShell
): string {
