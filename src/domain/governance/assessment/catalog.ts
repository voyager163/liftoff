...
export const assessmentInventoryContract = {
  families: 17,
  controls: 30,
  supported: 20,
  unsupported: 10,
  alwaysApplicableUnsupported: 6
} as const;
...
export function validateAssessmentCatalog(
  ...
  if (families.length !== assessmentInventoryContract.families ||
  ...
  if (
    controls.length !== assessmentInventoryContract.controls ||
    controls.filter((control) => control.supported).length !== assessmentInventoryContract.supported ||
    controls.filter((control) => !control.supported).length !== assessmentInventoryContract.unsupported ||
    controls.filter((control) => !control.supported && control.applicability === 'always').length !==
      assessmentInventoryContract.alwaysApplicableUnsupported
  ) {
    throw new Error('Assessment catalog support coverage does not match the release-owned inventory contract.');
  }
  return {
    schemaVersion: 1,
