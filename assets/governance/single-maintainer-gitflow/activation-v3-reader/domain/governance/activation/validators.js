...
const githubRemoteWriteMutations = new Set               ([
...
const governanceScopeSet = new Set        (governanceScopes);
...
function safePathParts(value         , path        )           {
  return validateArtifactPathParts(value, path);
}

function publicJson(value         , path        , depth = 0)          {
...
function validateExternalOperation(value         , path        )                         {
...
function validateFileChanges(value         , path        )                      {
...
function validateGitInput(value         , path        )                                                       {
...
function validateOutputBindings(value         , path        )                      {
...
export function validateEvidenceHeader(value         )                 {
  ...
  return {
    ...
    ...(header.scope === undefined ? {} : {
      scope: enumValue                 (header.scope, new Set([phaseScope(phaseId)]), 'evidenceHeader.scope')
    }),
...
function validateTransitionOperation(value         , path        )                      {
  ...
  const effects = operation.effects === undefined ? undefined : (() => {
    ...
    return operation.effects.map((entry, index) => {
      ...
      return {
        mutationClass: enumValue               (effect.mutationClass, new Set        (mutationClasses), `${path}.effects[${index}].mutationClass`),
...
export function validateSavedTransitionPlan(value         )                      {
  ...
  if (plan.approvalBundle !== undefined) {
    ...
    approvalBundle = plan.approvalBundle.map((entry, index) => {
      ...
      const id = enumValue         (item.phaseId, phaseIdSet, 'transitionPlan.approvalBundle.phaseId');
  ...
  return {
    schemaVersion: 2,
    scope: enumValue                 (plan.scope, new Set([phaseScope(phaseId)]), 'transitionPlan.scope'),
    phaseId,
    ...
    mutationClasses: {
      local: stringArray(mutationClassesValue.local, 'transitionPlan.mutationClasses.local').map((entry) =>
        enumValue               (entry, new Set        (mutationClasses), 'transitionPlan.mutationClasses.local')
      ),
      remote: stringArray(mutationClassesValue.remote, 'transitionPlan.mutationClasses.remote').map((entry) =>
        enumValue               (entry, new Set        (mutationClasses), 'transitionPlan.mutationClasses.remote')
      )
    },
    operations,
    approval: {
...
export function validateGraphReconciliationRecord(
  value         ,
  recognizedGraphHashes                      = new Set([canonicalPhaseGraphHash])
)                            {
