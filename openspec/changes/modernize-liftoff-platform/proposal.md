...
- Extend native encrypted private-state execution from the existing macOS implementation to Linux and Windows, including observed storage protection, principal-bound key custody, exact OpenTofu locking/write semantics, protected process I/O, settlement and recovery. Preserve original macOS identities, records, keys and approvals; fixtures or removal of a platform guard cannot substitute for the new host adapters or their native evidence.
- Add separately selected, approval-bound Linux keystore enrollment and restart recovery for an isolated Liftoff-owned store and controlled daemon lifecycle. Do not adopt the ordinary desktop keyring or infer durable encrypted custody from read-only Secret Service discovery. Readiness requires authoritative persisted-store binding, protected key identity and fresh-process readback; enrollment, unlock, key-read and disposal authority remain distinct from ordinary activation or installation.
- Add a separately versioned, explicitly selected Linux restart-guard profile permitting only `WRITE_FILE` on identity-verified `/dev/null` (character device major 1, minor 3). Preserve the existing strict profile and its identity; persisted-store writes, additional rights on other devices, device creation and inherited-descriptor bypasses remain forbidden. This narrowly scoped sink supports native D-Bus startup, not general writable access to `/dev`.
...
- Approval of cross-platform private-state implementation does not authorize creating encrypted volumes, changing ACLs or host policy, accessing real key stores or credentials, moving existing state, or provisioning qualification/cloud resources. Those effects require separately bounded authorization; unavailable host interfaces or native evidence remain explicit implementation/qualification blockers.
- The controlled Linux store design adds implementation scope only. No daemon is launched, password requested, key enrolled, existing collection unlocked or store disposed during planning; no passphrase, key bytes or password verifier belongs in public records or model context.
- Approval of the null-device profile revises implementation scope only. It does not authorize keystore operations, host-policy or device-node changes, cloud effects or publication, and does not turn source-fixture results into native custody or release qualification.
