...
**Reason**: The coordinated release replaces current npm delivery with self-contained native channels and deliberately has no final npm bridge.
**Migration**: Use Homebrew on macOS, WinGet on Windows, and verified direct native installation on Linux. Preserve public historical explicit-version npm installation under the retained historical contract.

### Requirement: Release automation verifies package integrity before publishing
**Reason**: There is no future npm publication to authorize.
**Migration**: Move current build, lockfile, test, package-inventory, licensing, relocation, and installed-command checks to native artifact qualification on every declared platform and the coordinated publication gate. Keep historical tarball verification available without rebuilding or republishing history.

### Requirement: Stable releases use the latest npm dist-tag
**Reason**: npm `latest` cannot be current release authority when new versions are native-only.
**Migration**: Use the authoritative signed native release manifest and verified owner-channel availability. Keep historical npm tags and explicit versions as historical references without retagging them to imply native delivery.

### Requirement: npm publishing is explicit and authenticated
**Reason**: Future npm trusted-publication duties are retired, not extended to a bridge package.
**Migration**: Bind native release signing/provenance and owner-channel publication to the canonical public source and coordinated gate. Retain historical trusted-publishing attestations, public package access, licensing, and original commit identities.

### Requirement: Documentation presents the global install path
**Reason**: Global npm installation and a global Node.js floor are no longer the primary native CLI setup path.
**Migration**: Document the platform-native channels, exact version verification, first-use commands, separate contributor/project toolchains, and explicit npm-to-native installation migration. Label npm instructions as historical verification or separately approved recovery, not current onboarding.
