...
The system SHALL generate Power Apps application files from an explicit snapshot of Microsoft's `PowerAppsCodeApps/templates/starter` at a tested immutable commit packaged with the Liftoff release. The package SHALL record the upstream repository, template path, commit, explicit file list, content hashes, license attribution, and a tested deterministic lockfile. Runtime generation SHALL NOT fetch mutable upstream content.
...
- **AND** it does not clone, download, or resolve the upstream `main` branch
