...
- [ ] 9.2 Use `.github/workflows/ci.yml` for macOS, Linux, and Windows acceptance, including the new preview/history/migration/recovery cases in explicit Windows boundary coverage where needed; verify native paths, case/link handling, process interruption, and the existing package-check/package-smoke lanes pass without adding a test framework or publishing. Previously deferred for local-only delivery; the user has now authorized a feature branch, commit, push, and the existing CI workflow. Awaiting cross-platform results.
