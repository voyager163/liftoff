...
- **THEN** the repository contains the source corresponding to the supported native release and preserves the history for historical npm releases
...
- **THEN** the CLI builds, its tests run, and the self-contained CLI executes outside the checkout without any Mission Control workspace files
...
- **THEN** filesystem-sensitive tests and native installation checks use platform-correct path handling
...
- **AND** native bundled-runtime and dependency notices are included as required
