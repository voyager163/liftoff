...
Define the self-contained Liftoff CLI lifecycle for creating, assessing, adopting, maintaining, governing, and inspecting supported projects while preserving independent installation, project, and provider authority.
...
- **AND** supported activation migration stays under update while adopt and fresh-target migrate remain distinct
...
- **THEN** it explains project-read-only preview, external receipts, exact default-No approval without manual hash entry, protected production files, create-only expansion, preserved history, partial recovery, and removed `--apply`
...
- **THEN** Liftoff exits 0 and describes owner-aware CLI replacement, non-installing check, JSON, dedicated-command authorization, separately approved legacy handover, and separation from project update
...
- **THEN** it identifies running CLI/executable, policy 8, credential-policy schema 2, activation contract 4, graph schema 3, current schema identities, and actual graph digest without a separate setup-skill version
...
- **THEN** verification reports consistency with incomplete not-started selected scope, `ok: false`, and exit 2
...
- **THEN** verification exits 1 with inconsistent, incomplete, indeterminate selected scope and `ok: false`
...
- **THEN** local-scoped verify exits 0 with `ok: true`, local completion, and separately reported pending activation

#### Scenario: Only one local phase is complete
- **WHEN** local state is consistent but further local work remains
- **THEN** local verify exits 2 with `consistent: true`, `complete: false`, and `ok: false` without calling the project inconsistent or complete
